...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      "rounded-2xl border border-border/60 bg-card/50 px-6 transition-all duration-300 hover:border-border",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-5 font-medium transition-all duration-300 text-left hover:text-primary [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
